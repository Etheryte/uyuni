...
function onNodeClick(this: unknown, d) {
...
function appendPatchStatus(cell, patchCountsArray) {
  if (DEPRECATED_unsafeEquals(patchCountsArray, undefined)) {
...
function updateDetailBox(d) {
  ...
  const contentWrapper = detailBox.append("div").classed("content-wrapper", true);
  contentWrapper
    .append("a")
    .attr("href", "#")
  ...
  if (Utils.isCompliantToSSM(d)) {
    ...
    row.append("td").text(t("Add/remove system from SSM"));
    const ssmCell = row.append("td");
    ssmCell
      .append("button")
  ...
  function appendSimpleRow(key, valueFn) {
    ...
    valueFn(row.append("td").append("strong"));
  }
...
function myDeriveClass(node) {
  ...
  if ((window.view === "proxy-hierarchy" && node.data.type === "proxy") || node.data.type === "group") {
    return "inner-node";
  }
